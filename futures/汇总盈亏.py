...
if __name__ == '__main__':
    ...
    for i in varlist:
        try:
            var = i
            print(i)
            f = flows(futures, start, end, var)
            print(i,"ok")
            f = f[['variety', 'BrokerID', 'corr', 'corr2', 'today_net', 'net_chg', 'corr3', 'cumpnl']].sort_values(
            ...
            # flow = flow_pnl.flows(futures, start, end, var)
            # flow = flow.sort_values('cumpnl', ascending=False)
            # print(flow[['date', 'variety', 'BrokerID', 'corr', 'corr2', 'corr3', 'cumpnl']].sort_values('cumpnl',
            #                                                                                             inplace=False))
