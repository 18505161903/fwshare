...
def flows2(futures, start=None, end=None, var=None):
    ...
    long = position.groupby(['date', 'variety', 'long_party_name'])[
        ['long_open_interest', 'long_open_interest_chg']].sum()
    # print(long)
    short = position.groupby(['date', 'variety', 'short_party_name'])[
        ['short_open_interest', 'short_open_interest_chg']].sum()
    # # 合并
    frames = [long, short]
    ...
    for i in party_name:
        try:
            ...
            mem['profit'] = mem.apply(
                lambda x: abs(x['tomorrow_chg']) if x['tomorrow_chg'] * x['net_chg'] > 0 else -abs(x['tomorrow_chg']),
                axis=1)
            mem['交易信号'] = mem.apply(lambda x: 0 if x['net_chg'] == 0 else 1 if x['net_chg'] > 0 else -1, axis=1)
            mem = mem.dropna()

            # mem = mem[(mem['BrokerID']=="永安期货")]
            ...
            df = df.append(df1)
            print(df)
            # mainSignal.insert(json.loads(df1.T.to_json()).values())
            # print(json.loads(df1.T.to_json()).values())
        except:
            continue
    return df
...
if __name__ == '__main__':
    ...
    print(flows.sort_values('corr2', ascending=False))